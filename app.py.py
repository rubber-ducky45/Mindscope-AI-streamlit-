...
st.markdown(
    ...
    /* Blur the contents, not the whole card shell, so the effect is obvious. */
    .result-card.loading > * {
        filter: blur(7px);
        opacity: .62;
        user-select: none;
    ...
    /* Soft frosted veil while analysis is running. */
    .result-card.loading::after {
        content: "";
        position: absolute;
        inset: 0;
        border-radius: inherit;
        background: rgba(255, 250, 246, .30);
    ...
    .firefly {
        ...
        font-size: 1.35rem;
        line-height: 1;
        filter: drop-shadow(0 0 6px rgba(255, 230, 109, .75));
...
def render_fireflies():
    ...
    smileys = ["😊", "😀", "😄", "😁", "✿", "❀", "❁", "❃", "✾"]
    dots = "".join(
        f'<span class="firefly" style="left:{x}%;top:{y}%;animation-delay:{delay}s">{smileys[i % len(smileys)]}</span>'
        for i, (x, y, delay) in enumerate(positions)
    )
    st.markdown(
        f"""
...
if mode == "Story Mode":
    ...
    with story_tabs[2]:
        ...
        with note_col:
            ...
            st.dataframe(
                metrics_df[["Model", "Accuracy", "F1"]].style.format(
                    {"Accuracy": "{:.1%}", "F1": "{:.1%}"}
                ),
                use_container_width=True,
                hide_index=True,
    ...
    with story_tabs[3]:
        ...
        with ending_right:
            ...
            if st.session_state.show_review:
                st.success(
                    "The cards fade into flowers and little smiley faces dance through the meadow."
                )
                st.dataframe(
                    selected_row[
                        [
                ...
                st.markdown(
                    html_card(
                        ...
                        <p class="small-muted">
                            The dancing smileys and flowers will settle as the data appears.
                        </p>
                        """,
                    ),
                    unsafe_allow_html=True,
